<script>
	...
	function selectActivity(e){
		for(let i=0; i<e.path.length; i++){
			if (e.path[i].className.includes('activityContainer')) {
				selectedIndex = activities.findIndex(x => x.id == e.path[i].id);
	...
	function removeActivity(e){
		activities = activities.filter(x => x.id != e.path[1].id);
		saveActivities();
	}
...
<div>
	...
	<div class="activityContainer" id={activity.id} on:click={selectActivity}>
		<button class="badbutton" id="remove" on:click={removeActivity}>Trash</button>
		<h4>
			{activity.name}
		</h4>
		<p>
